...
#define AUTO_UPDATE_NEED	1
#define AUTO_UPDATE_NONEED	0
#define FTS_UPGRADE_AA		0xAA
#define FTS_UPGRADE_55		0x55
#define HIDTOI2C_DISABLE					0
#define FTXXXX_INI_FILEPATH_CONFIG "/sdcard/"
#define FT5436_OFILM_POS	0
#define FT5436_MUTTO_POS	1
#define FT5346i_OFILM_POS	2
#define FT5346i_MUTTO_POS	3
#define FT5436_POS	0
#define FT5346i_POS	2
...
* Static variables
*******************************************************************************/
static unsigned char FT5346i_FW_OFILM[] = {
	//#include "FT_5x46_OFILM_app.i"
	//#include "Lenovo_Karate_PVT_0100000F_20160701_app.i"
	#include "Lenovo_Karate_PVT_01000010_20160706_app.i"
};
...
static unsigned char FT5436_FW_OFILM[] = {
	...
	//#include "Lenovo_KaratePlus_Ofilm_V13_20160612_app.i"
	//#include "Lenovo_KaratePlus_Ofilm_V14_20160701_app.i"
	#include "Lenovo_KaratePlus_Ofilm_V15_20160726_app.i"
};
...
{
	{0x54,FTS_MAX_POINTS_5,AUTO_CLB_NONEED,AUTO_UPDATE_NEED,30, 30, 0x54, 0x2c, 10, 2000,
		FT5436_FW_OFILM, sizeof(FT5436_FW_OFILM)}, //,"FT5436 ofilm"
	{0x54,FTS_MAX_POINTS_5,AUTO_CLB_NONEED,AUTO_UPDATE_NEED,30, 30, 0x54, 0x2c, 10, 2000,
		FT5436_FW_MUTTO, sizeof(FT5436_FW_MUTTO)}, //,"FT5436 mutto"

	{0x54,FTS_MAX_POINTS_5,AUTO_CLB_NONEED,AUTO_UPDATE_NEED,2, 2, 0x54, 0x2c, 20, 2000,
		FT5346i_FW_OFILM, sizeof(FT5346i_FW_OFILM)}, //,"FT5346i ofilm"
	{0x54,FTS_MAX_POINTS_5,AUTO_CLB_NONEED,AUTO_UPDATE_NEED,2, 2, 0x54, 0x2c, 20, 2000,
		FT5346i_FW_MUTTO, sizeof(FT5346i_FW_MUTTO)}, //,"FT5346i mutto"
};
/*******************************************************************************
...
* Input: const char *fwname
* Output: get ic info in fts_updateinfo_curr
* Return: no
***********************************************************************/
int fts_get_upgrade_array(const char *chip_name)
{
	u8 chip_id;
	u8 fw_vendor_id;
	int ret = 0;
	...
	ret = fts_read_reg(fts_i2c_client, FTS_REG_FW_VENDOR_ID, &fw_vendor_id);
	if (ret<0) {
		printk("[Focal][Touch] read value fail");
		return -1;
	}
	if(fw_vendor_id != FTS_FW_VENDOR_ID_OFILM && fw_vendor_id != FTS_FW_VENDOR_ID_MUTTO) {
		/* Get the chip update infos, only for read the vendor id from boot domain */
		if (0 == strncmp(chip_name, "ft5346i", strlen("ft5346i"))) {
			pr_info("%s: chip is %s. update %d\n", __func__, chip_name, FT5346i_POS);
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[FT5346i_POS], sizeof(struct fts_Upgrade_Info));
		}

		if (0 == strncmp(chip_name, "ft5436", strlen("ft5436"))) {
			pr_info("%s: chip is %s. update %d\n", __func__, chip_name, FT5436_POS);
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[FT5436_POS], sizeof(struct fts_Upgrade_Info));
		}

		/* Read vendor id from boot, next step will get the right update info by vendor id */
		printk("%s unknown vendor_id = 0x%x, read vendor id from boot.\n", __func__, fw_vendor_id);
		fts_ctpm_fw_upgrade_ReadVendorID(fts_i2c_client, &fw_vendor_id);
		printk("%s fw_vendor_id = 0x%x\n", __func__, fw_vendor_id);
	}

	printk("%s chip_id = 0x%x fw_vendor_id = 0x%x\n", __func__, chip_id, fw_vendor_id);
	if (0 == strncmp(chip_name, "ft5346i", strlen("ft5346i"))) {
		if (FTS_FW_VENDOR_ID_OFILM == fw_vendor_id) {
			pr_info("%s: chip is %s. update %d\n", __func__, chip_name, FT5346i_OFILM_POS);
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[FT5346i_OFILM_POS], sizeof(struct fts_Upgrade_Info));
			return 0;
		}
		if (FTS_FW_VENDOR_ID_MUTTO == fw_vendor_id) {
			pr_info("%s: chip is %s. update %d\n", __func__, chip_name, FT5346i_MUTTO_POS);
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[FT5346i_MUTTO_POS], sizeof(struct fts_Upgrade_Info));
			return 0;
		}
	}
	...
	if (0 == strncmp(chip_name, "ft5436", strlen("ft5436"))) {
		if (FTS_FW_VENDOR_ID_OFILM == fw_vendor_id) {
			pr_info("%s: chip is %s. update %d\n", __func__, chip_name, FT5436_OFILM_POS);
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[FT5436_OFILM_POS], sizeof(struct fts_Upgrade_Info));
			return 0;
		}
		if (FTS_FW_VENDOR_ID_MUTTO == fw_vendor_id) {
			pr_info("%s: chip is %s. update %d\n", __func__, chip_name, FT5436_MUTTO_POS);
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[FT5436_MUTTO_POS], sizeof(struct fts_Upgrade_Info));
			return 0;
		}
	}

	pr_err("%s: cannot find right chip updateinfo\n", __func__);
	return -1;
}
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	u8  	bt_ecc;
	int	i_ret;
	int	fw_filenth = fts_updateinfo_curr.fw_size;

	if(fts_updateinfo_curr.fw[fw_filenth-12] == 30)
	{
		is_5336_fwsize_30 = 1;
	}
	else 
...
* Return: file size
***********************************************************************/
static int fts_GetFirmwareSize(const char *firmware_name)
{
...
note:the firmware default path is sdcard.
	if you want to change the dir, please modify by yourself.
*/
static int fts_ReadFirmware(const char *firmware_name,unsigned char *firmware_buf)
{
	struct file *pfile = NULL;
	struct inode *inode;
	unsigned long magic;
...
* Return: success =0
***********************************************************************/
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, const char *firmware_name)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
...
{
	u16 ui_sz;
	ui_sz = fts_updateinfo_curr.fw_size;

	if (ui_sz > 2) {
		if(fts_updateinfo_curr.CHIP_ID==0x36  || fts_updateinfo_curr.CHIP_ID==0x86  || fts_updateinfo_curr.CHIP_ID==0x64)
			return fts_updateinfo_curr.fw[0x10a];
		else if(fts_updateinfo_curr.CHIP_ID==0x58)
			return fts_updateinfo_curr.fw[0x1D0A];
		else
			return fts_updateinfo_curr.fw[ui_sz - 2];
	}
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = fts_updateinfo_curr.fw_size;
	...
	*/
	if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) ||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14)
		...
		if ((fts_updateinfo_curr.fw[fw_len - 8] ^ fts_updateinfo_curr.fw[fw_len - 6]) == 0xFF
			&& (fts_updateinfo_curr.fw[fw_len - 7] ^ fts_updateinfo_curr.fw[fw_len - 5]) == 0xFF
			&& (fts_updateinfo_curr.fw[fw_len - 3] ^ fts_updateinfo_curr.fw[fw_len - 4]) == 0xFF) {
			/*FW upgrade */
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	fts_read_reg(client, FTS_REG_ID, &chip_id);
	fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	dev_info(&client->dev, "[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x, tp_chip_id = 0x%x, host_chip_id = 0x%x\n", uc_tp_fm_ver, uc_host_fm_ver, chip_id,  fts_updateinfo_curr.CHIP_ID);
	if (chip_id !=  fts_updateinfo_curr.CHIP_ID || uc_tp_fm_ver < uc_host_fm_ver ) {
		msleep(100);
